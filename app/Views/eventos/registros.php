...
<script>
    const data_registros = <?php echo json_encode($registros); ?>;

    const obtenerMes = (mes) => {
        let miFecha = new Date();
        if (0 < mes && mes <= 12) {
            miFecha.setMonth(mes - 1);
    ...
    $(document).ready(function () {
        ...
        $('#registros_table').DataTable({
            ...
            columns: [
                ...
                { data: 'nombreCompleto', render: (data, type, row) => {
                        return `
                            <h6 class="fs-16 font-w600 mb-0">${data}</h6>
                            <span class="fs-14">${row.empresa}</span>
                        `
                    } 
                },
                { data: 'day', render: (data, type, row) => {
                        return `
                            <h6 class="fs-16 text-black font-w400 mb-0">${row.day} de ${obtenerMes(row.month)} del ${row.year}</h6>
                            <span class="fs-14">${row.hour}</span>
                        `
                    }
                },
                { data: 'userRegistration', render: (data, type, row) => {
                        return `
                            <span class="fs-16 text-black font-w500">${data}</span
                        `
                    }
                },
                { data: 'type', render: (data, type, row) => {
                        return `
                            <span class="text-${data === "Entrada" ? "success" : "danger"} fs-16 font-w500 text-end d-block">${data}</span>
                        `
                    }
                }
            ],
        ...
        let btn2 = createElementFromHTML('<button onclick="descargarReporte()" class="btn btn-success ms-2 m-2"><i class="bi bi-file-excel-fill"></i> Generar reporte</button>')
        document.querySelector('.dt-search').append(btn2);
    });

    const descargarReporte = () => {
        try {
            // validando que si venga el reporte
            const reporteBase64 = "<?php echo isset($reporte) ? $reporte : ''; ?>";
            if (!reporteBase64) {
            ...
            let byteCharacters;
            try {
                byteCharacters = atob(reporteBase64);
            } catch (error) {
                throw new Error('El formato del reporte es inválido (error en base64).');
            }
